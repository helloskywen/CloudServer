...
public class Main {	
	...
	public static float opt3001_CalcIlluminance(int raw_val){
		int e, m;
		m = raw_val & 0x0FFF;
		e = (raw_val & 0xF000) >> 12;
		return (float)(m * (0.01 * Math.pow(2, e)));
	}
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException, SQLException{
		...
		sql="create table t_sensor_3(time DateTime not null, building_number int not null, building_level int not null, room int not null, device_type varchar(20) not null, "
		...
		rs = meta.getTables(null, null, "t_sensor_3", null);
		if(rs.next() == false)
			stat.execute(sql);
		
		sql="create table t_sensor_4(time DateTime not null, building_number int not null, building_level int not null, room int not null, device_type varchar(20) not null, "
				+ "device_number int not null, data_type varchar(16) not null, value float, primary key(time,building_number,building_level,room,device_type,device_number,data_type)) DEFAULT   CHARSET=utf8";
		
		rs = meta.getTables(null, null, "t_sensor_4", null);
		if(rs.next() == false)
			stat.execute(sql);
			...
			if(data_type == 0x0A){//sensor measure value
				if(dev_type == 0x94){
					...
					if(d[2] !=0 && d[3] !=0){
						...
						sql = "insert into t_sensor_3 values(" + "\"" + dateTime + "\"" + "," + "\"" + building_number + "\"" + "," + "\"" + building_level + "\"" + "," + "\"" + room + "\"" + "," + "\"" + sql_device_type + "\"" + "," + "\"" + dev_number + "\"" + "," + "\"" + sql_data_type + "\"" + "," + "\"" + temperatureC + "\"" + ")"; 
						stat.executeUpdate(sql);
						
						sql_data_type = "humidityRH";	
						sql = "insert into t_sensor_3 values(" + "\"" + dateTime + "\"" + "," + "\"" + building_number + "\"" + "," + "\"" + building_level + "\"" + "," + "\"" + room + "\"" + "," + "\"" + sql_device_type + "\"" + "," + "\"" + dev_number + "\"" + "," + "\"" + sql_data_type + "\"" + "," + "\"" + humidityRH + "\"" + ")"; 
						stat.executeUpdate(sql);						
					} else {
						int illuminance_raw_val = d[0]<<8 | d[1];
						float illuminance = opt3001_CalcIlluminance(illuminance_raw_val);
						System.out.println(dateTime + ", " + building_number + ", " + building_level + ", " + room + ", " + dev_type + ", " + dev_number + ", I(centilux): " + (int)(illuminance * 100));
						sql_data_type = "illuminance";
						sql = "insert into t_sensor_3 values(" + "\"" + dateTime + "\"" + "," + "\"" + building_number + "\"" + "," + "\"" + building_level + "\"" + "," + "\"" + room + "\"" + "," + "\"" + sql_device_type + "\"" + "," + "\"" + dev_number + "\"" + "," + "\"" + sql_data_type + "\"" + "," + "\"" + (int)(illuminance * 100)+ "\"" + ")"; 
						stat.executeUpdate(sql);
					}
				}				
				...
				if(dev_type == 0xd0){
					...
					sql = "insert into t_sensor_4 values(" + "\"" + dateTime +"\"" + "," + "\"" + building_number + "\"" + "," + "\"" + building_level + "\"" + "," + "\"" + room + "\"" + "," + "\"" + sql_device_type + "\"" + "," + "\"" + dev_number + "\"" + "," + "\"" + sql_data_type + "\"" + ","  + "\"" + value + "\"" + ")";
					stat.executeUpdate(sql);					
